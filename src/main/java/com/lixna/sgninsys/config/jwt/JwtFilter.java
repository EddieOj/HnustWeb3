...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
        ...
        CustomToken token = (CustomToken) createToken(request, response);
        if (token == null) {
    ...
    protected AuthenticationToken createToken(ServletRequest request, ServletResponse response) {
        ...
        String loginType = req.getHeader(JwtConst.LOGIN_TYPE);
        CustomToken token = new CustomToken(authorization,loginType);
        return token;
    }
    ...
    protected boolean onAccessDenied(ServletRequest servletRequest, ServletResponse servletResponse) throws Exception {
        ...
        writer.write("{\"errCode\": 401, \"msg\": \"token不存在\"}");
